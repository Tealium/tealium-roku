...
ifndef ZIP_EXCLUDE
  ZIP_EXCLUDE= -x \*.git/\* -x \*.project -x \*.pkg -x storeassets\* -x keys\* -x \*/.\*
endif
...
# do not zip up Makefiles, or any files ending with '~'
	...
	then \
		...
		(zip -9 -r "$(ZIPREL)/$(APPNAME).zip" . -x \*~ -x \*.png -x \*.git/\* -x \*.project  -x Makefile $(ZIP_EXCLUDE)); \
	else \
...
install: $(APPNAME)
	...
	@curl -v --digest -u $(USER):$(PASS) -F "mysubmit=Install" -F "archive=@$(ZIPREL)/$(APPNAME).zip" http://$(ROKU_DEV_TARGET)/plugin_install | grep "<font color" | sed "s/<font color=\"red\">//"
